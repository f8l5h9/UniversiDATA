...
tab2 <- as.data.frame(cbind(table(bd$abandono,bd$des_genero),
                        table(bd$abandono,bd$adscrito),
                        table(bd$abandono,bd$doble),
                        table(bd$abandono,bd$cod_universidad),
                        table(bd$abandono,bd$Rama)))
zx <- tab2
tab2 <- t(100*t(t(tab2)/colSums(tab2)))
tab2 <- cbind((t(zx)),tab2) 
kbl(tab2, col.names = c("$N (TR \\geq 0,4)$","$N (TR<04)$", "$\\% (TR \\geq 0,4))$","$ \\% TR<0,4$"), caption = "Número estudianes (N) y distribución porcentual en base a TR04 \\label{tab:estudiantes}", 
...
footnote(general = "Porcentajes calculados sobre suma elementos fila")
```
