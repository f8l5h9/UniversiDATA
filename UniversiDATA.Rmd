...
```{r leer datos matricula, warning = FALSE, message = FALSE}
# UCM
# ================
...
formula <- abandono ~ des_genero + doble + des_rama + adscrito + cod_universidad + 
  n.titula + n.acceso + edad.media.tit + muni.local + prov.local + nota.mediana + nota.media + nota.min + uni.padre + uni.madre + Profesor.Asociado + Profesor.Ayudante.Doctor
mymars <- earth(formula, data = bd, minspan=100, endspan = 
                  100, degree = 2, glm=list(family=binomial))
summary(mymars)
library(caret)
xx <- as.factor(ifelse(predict(mymars) > 0.12, 1, 0))
levels(xx) <- c("NO","SI")
confusionMatrix(xx, bd$abandono)

plotmo(mymars)
```

```{r graficos}
plotmo(mymars,caption="", do.par=FALSE,npoints =F,pt.col="blue", cex.response=1,lwd.degree1=1, jitter = 1, grid.col = TRUE)
```

```{r logit como mars}
mymars.logit <- glm(bd$abandono ~ mymars$bx[,-1],family = "binomial")
summary(mymars.logit)
```
...
formula <- abandono ~ des_genero + doble + des_rama + adscrito + cod_universidad + 
  n.titula + n.acceso + edad.media.tit + muni.local + prov.local + nota.mediana + nota.media + nota.min + uni.padre + uni.madre + Profesor.Asociado + Profesor.Ayudante.Doctor
mylogit <- glm(formula ,family=binomial(link='logit'),data = bd)
summary(mylogit)
library(caret)
xx <- as.factor(ifelse(predict(mylogit) > 0.12, 1, 0))
levels(xx) <- c("NO","SI")
confusionMatrix(xx, bd$abandono)
```
...
library(tictoc)
tic()
fit <- earth(formula, data = bd, degree = 2, minspan = 500, 
             endspan = 500, pmethod="cv", ncross=3, nfold=5, trace=.5,
             glm = list(family=binomial))
toc()
plot(fit, which=1, col.rsq=0, ylim=c(0,0.1))
```
...
set.seed(123)
library(rsample)
train <- initial_split(bd, prop = 0.7, strata = abandono)
# bd_train <- bd[train,]
# bd_test  <- bd[-train,]
bd_train <- training(train)
bd_test <- testing(train)
mymars.train <- earth(formula, data = bd_train, minspan=-2, 
                      endspan = 500,degree=2, 
                      glm=list(family=binomial))
# Error de test del modelo
# ==============================================================================
predicciones <- predict(mymars.train, newdata = bd_test, type = "response")
class_pred <- ifelse(predicciones > 0.5, 1, 0)
...
# Trazar la curva ROC
plot(perf, col = "red", main = "Curva ROC", lwd = 2)
# Calcular el AUC
...
#
library(caret)
xx <- as.factor(ifelse(predict(mymars) > 0.5, 1, 0))
levels(xx) <- c("NO","SI")
confusionMatrix(xx, bd$abandono)
```
...
```{r}
# https://bradleyboehmke.github.io/HOML/mars.html#prerequisites-5
# https://www.newtral.es/financiacion-publica-universidades/20240719/
library(dplyr)
hyper_grid <- expand.grid(
  degree = 1:2, 
  nprune = seq(15,30, length.out = 5) %>% floor()
)
...
ms <- 50
mb <- 10
control <- rpart.control(minsplit = ms,minbucket = mb, cp = 0.002)
arbol_0 <- rpart(formula , data = bd, method = 'anova', control = control)
layout(matrix(c(0,1,1,0,2,2,3,3), 2, 4, byrow = TRUE))
rpart.plot(arbol_0,tweak=1,digits = 3)
```

```{r}
set.seed(123)
train <- sample(1:nrow(bd), size = .7*nrow(bd))
bd_train <- bd[train,]
bd_test  <- bd[-train,]

# El árbol se crece al máximo posible para luego aplicar el pruning
library(tree)
control <- tree.control(dim(bd)[1], 
                        mincut = 1, 
                        minsize = 2,
                        mindev  = 0)
arbol_regresion_base<- tree(formula = formula,
                            data    = bd,
                            control = control)
